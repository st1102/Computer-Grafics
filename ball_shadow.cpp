...
public:
	...
	// x と z の値から床の色を返す（格子模様になるように）
	  ...
	  // ★ x, z の値によって(1.0, 1.0, 0.7)または(0.6, 0.6, 0.6)のどちらかの色を返すようにする
	    if(((int)(x/100)%2 + (int)(z/100)%2) % 2 == 0){
	      if(x < 0){
...
void getPixelColor(double x, double y, Vector3d &colorVec) {
  ...
  if(t_sphere > 0) { // 球との交点がある
    ...
    Vector3d p = viewPosition + t_sphere * ray; //交点
    Vector3d N = p - center; //法線			
    N.normalize();
    lightDirection.normalize();
    double cosa = N * (-lightDirection);
    if(cosa < 0){
      Id = 0;
    } else {
      Id = Iin * Kd * cosa;
    ...
    Vector3d R = lightDirection + 2 * cosa * N; //正反射方向
    R.normalize();
    Vector3d v = viewPosition - p;
    v.normalize();
    double cosb = R * v;
    double n = 10;
    if(cosb < 0){
  ...
  if(t_board > 0) { // 床との交点がある
    ...
    Vector3d p = viewPosition + t_board * ray; //交点
    
    //交点から光源へのレイ
    Vector3d ray_light = -lightDirection;
      
    ray_light.normalize(); // 交点からのレイの長さの正規化
    
    double t_light_sphere = sphere.getIntersec(p, ray_light);
    ...
    Vector3d N = Vector3d(0, 1 ,0); //法線 （真上）		
    N.normalize();
    lightDirection.normalize();
    double cosa = N * (-lightDirection);
    if(cosa < 0){
    ...
    double I = Id + Is + Ia;
    r = std::min(I * board.getColorVec(t_board*ray.x, t_board*ray.z).x, 1.0); // 1.0 を超えないようにする
    g = std::min(I * board.getColorVec(t_board*ray.x, t_board*ray.z).y, 1.0); // 1.0 を超えないようにする
    b = std::min(I * board.getColorVec(t_board*ray.x, t_board*ray.z).z, 1.0); // 1.0 を超えないようにする

    
    if(t_light_sphere > 0){ //球の影になる
      r *= 0.5;
      g *= 0.5;
      b *= 0.5;
      }
...
void display(void) {
  ...
  for(int y = (-halfHeight); y <= halfHeight; y++ ) {
    for(int x = (-halfWidth); x <= halfWidth; x++ ) {
      ...
      Vector3d colorVec;
      Vector3d color_sample;

      //ここでオバーサンプリングを行う
      for(int i = 0; i < 3; i++){
	for(int j = 0; j < 3; j++){
	  getPixelColor(x + i * 1.0/3, y + j * 1.0/3, color_sample);
	  colorVec += color_sample;
	}
      }
      colorVec = colorVec * 1.0/9; //.0を付ける
